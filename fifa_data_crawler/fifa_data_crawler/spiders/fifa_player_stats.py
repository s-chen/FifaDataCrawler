...
class FifaPlayerStatsSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        player_skill_moves = response.css('div.card > ul.pl > li').css('::text')[6].get().strip()
        player_international_reputation = response.css('div.card > ul.pl > li').css('::text')[10].get().strip()
        player_attacking_work_rate = response.css('div.card > ul.pl > li').css('::text')[15].get().split('/')[0].strip()
        player_defensive_work_rate = response.css('div.card > ul.pl > li').css('::text')[15].get().split('/')[1].strip()
        player_body_type = response.css('div.card > ul.pl > li').css('::text')[17].get()
        player_real_face = response.css('div.card > ul.pl > li').css('::text')[19].get()
        player_release_clause = response.css('div.card > ul.pl > li').css('::text')[21].get()
